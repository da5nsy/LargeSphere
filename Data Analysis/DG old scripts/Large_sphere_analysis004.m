...
%% CORRECTION
%Then plotting in CIE1931

if strcmp(obs,'DG')
    needCorrecting=[440,460,500,540,560,580,600,620,660,680,700];
    tops480 = mean(reshape([files(5).dataxy;files(6).dataxy;files(8).dataxy],2,160*3),2);
    bottom480 = mean(reshape(files(7).dataxy,2,160),2);
    
    diff480s = tops480-bottom480;
    
    for i=1:length(files)
        files(i).dataxycor=files(i).dataxy;
        if any(str2num(files(i).name(1:3))==needCorrecting)
            files(i).dataxycor=files(i).dataxy + repmat(diff480s,1,16,10);
        end
    end
    
    plotCIE(2,xbar2,ybar2,zbar2), hold on
    for i=1:length(files)
        xydata=mean(reshape(files(i).dataxycor,2,160),2);
        s=scatter(xydata(1,:),xydata(2,:),'r.');
        s.CData=[(str2num(files(i).name(1:3))-400)/300,0,0];
        s.SizeData=200;
        text(xydata(1,:),xydata(2,:),files(i).name(1:3))
    end
else
    plotCIE(2,xbar2,ybar2,zbar2), hold on
    for i=1:length(files)
        xydata=mean(reshape(files(i).dataxy,2,160),2);
        s=scatter(xydata(1,:),xydata(2,:),'r.');
        s.CData=[(str2num(files(i).name(1:3))-400)/300,0,0];
        s.SizeData=200;
        text(xydata(1,:),xydata(2,:),files(i).name(1:3))
