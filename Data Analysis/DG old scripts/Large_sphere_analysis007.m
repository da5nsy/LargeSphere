function Large_sphere_analysis007(obs,ciedata2,xbar2,ybar2,zbar2,lambdaCie2,HighL,LowL,repstart)
...
%% 007 Attempting to add elipses to data
...
for j=4:13%1:length(spectralambda)
    for i=1:length(files)
        %takes the first data set for each wavelength, ignores repeats
        if strcmp(files(i).name(1:3),num2str(spectralambda(j)))
            clear X %this is dirty as hell
            X(:,1)= reshape(files(i).datauv(1,HL:LL,repstart:end),...
                48,1);            
            X(:,2)=reshape(files(i).datauv(2,HL:LL,repstart:end),...
                48,1);      
